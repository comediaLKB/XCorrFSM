...
g=0.975;%0.975;% %Anisotropy factor

Dteta=0.01;% incident angle (rad) (N_teta+1)*px/(2*z+L)

%numerical parameters
N=3000; %matrix dimension (square)
N_theta=32; %number of angular computed values
N_diff=20; %number of diffuser layers
px=0.4; %pixel size (µm)
...
%     E_inf=FFT(E_s_new);
%     I_inf=abs(E_inf).^2;
% %     Ib(m)=I_inf(N/2+1,N/2+1)/N^4;
%     %computed angular scattering spread
%     ang(m)=sqrt( 1 - sum(I_inf(:).*cos(kr(:)*lambda)) / sum(I_inf(:)) );
...
% hold on
% plot(l_it,cross_corr,'b+','LineWidth',1)
ax2 = gca;
ax2.FontSize=14;
